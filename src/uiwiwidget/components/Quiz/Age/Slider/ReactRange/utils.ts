...
export function getTrackBackground({
      ...
      if (rtl && direction === Direction.Right) {
            ...
            direction = Direction.Right;
      } else if (rtl && Direction.fromCenter) {
            direction = Direction.fromCenter;
      }
      ...
      const middle = progress.reduce((acc, point, index) => {
            let ret = `${acc}, ${colors[index]} ${point}%, ${colors[index + 1]} ${point}%`;
            if (progress[0] < 50) {
                  ret = `${acc}, ${colors[index + 2]} ${point}%`;
                  direction = Direction.Left;
                  // css = `linear-gradient(${direction},  ${colors[0]} 0%${ret}, ${colors[0]} 50%, ${colors[0]} 100%)`;
                  // css = `linear-gradient(${direction},  ${colors[0]} 0%, ${colors[0]} 50%${ret}, ${colors[0]} 100%)`
                  // css = `linear-gradient(to right, transparent 0%, red  ${point}%, transparent 50%, transparent 100%)`;
            } else {
                  ret = `${acc}, ${colors[index + 1]} 50%, ${colors[index + 1]} ${point}%`;
                  // css = `linear-gradient(${direction},  ${colors[0]} 0%, ${colors[0]} 50%${ret}, ${colors[0]} 100%)`;
                  // css = `linear-gradient(to right,  transparent 0%, transparent 50%, #84BF06 50%, ${colors[index + 1]} ${point}%, transparent 50%)`
                  // css = `linear-gradient(to right,  transparent 0%, transparent 50%, ${colors[index + 1]} ${point}%, transparent 50%)`;
            }
            return ret;
      }, "");
      return css;
...
const getThumbWidth = (
      ...
      const width = Math.ceil(
            [thumbEl, ...Array.from(thumbEl.children)].reduce((width: number, el: Element) => {
            ...
            offsets.forEach(({ x: siblingX }, siblingIndex) => {
                  ...
                  if (
                        ...
                        ((thumbX >= siblingX && thumbX <= siblingX + siblingWidth) ||
                              (thumbX + thumbXWidth >= siblingX && thumbX + thumbXWidth <= siblingX + siblingWidth))
                  ) {
...
export const useThumbOverlap = (
      ...
      const [labelValue, setLabelValue] = useState(valueToLabel(values[index].toFixed(decimalPlaces)));
      ...
      useEffect(() => {
            if (rangeRef) {
                   ...
                   * 2, 3 and 4 also.
                   */
                  const overlaps = getOverlaps(index, offsets, thumbs, values, separator, decimalPlaces, valueToLabel);
                  ...
                  if (overlaps.length) {
                         ...
                         */
                        const offsetsX = overlaps.reduce<number[]>((a: number[], c: number, i: number, s: number[]) => {
                        ...
                        if (Math.min(...offsetsX) === offsets[index].x) {
                              ...
                              const lastWidth = thumbs[overlaps[offsetsX.indexOf(last)]].getBoundingClientRect().width;
