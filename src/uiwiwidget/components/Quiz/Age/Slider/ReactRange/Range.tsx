...
import { Direction, IProps } from "./types";
...
class Range extends React.Component<IProps> {
      ...
      componentDidMount() {
            ...
            this.props.values.forEach((value) => checkBoundaries(value, this.props.min, this.props.max));
            ...
            return this.getThumbs().map((thumb, index) => {
                  ...
                  switch (direction) {
                        case Direction.Right:
                              ...
                              thumbOffsets.x +=
                                    trackRect.width * relativeValue(values[index], min, max) - thumbRect.width / 2;
                              return thumbOffsets;
                        case Direction.Left:
                        ...
                        case Direction.Up:
                              thumbOffsets.x =
                                    ((thumbRect.width - trackRect.width) / 2 + thumbMargins.left + trackPadding.left) *
                                    -1;
                        ...
                        case Direction.Down:
                              ...
                              thumbOffsets.y = -trackPadding.left;
                              thumbOffsets.y +=
                                    trackRect.height * relativeValue(values[index], min, max) - thumbRect.height / 2;
                              return thumbOffsets;
                        default:
                              // @ts-ignore
      ...
      getTargetIndex = (e: Event) =>
            this.getThumbs().findIndex((child) => child === e.target || child.contains(e.target as Node));
      ...
      onMove = (clientX: number, clientY: number) => {
            ...
            const trackRect = trackElement.getBoundingClientRect();
            const trackLength = isVertical(direction) ? trackRect.height : trackRect.width;
            if (draggedTrackPos[0] !== -1 && draggedTrackPos[1] !== -1) {
                  ...
                  if (Math.abs(values[draggedThumbIndex] - newValue) >= step / 2) {
                        onChange(
                              replaceAt(values, draggedThumbIndex, this.normalizeValue(newValue, draggedThumbIndex))
                        );
                  }
            }
      };
      ...
      calculateMarkOffsets = () => {
            if (!this.props.renderMark || !this.trackRef || this.trackRef.current === null) return;
            ...
            for (let i = 0; i < this.numOfMarks + 1; i++) {
                  ...
                  if (this.props.direction === Direction.Left || this.props.direction === Direction.Right) {
                        res.push([
                              Math.round((trackWidth / this.numOfMarks) * i + paddingLeft - markWidth / 2),
                              -Math.round((markHeight - trackHeight) / 2)
                        ]);
                  } else {
                        res.push([
                              Math.round((trackHeight / this.numOfMarks) * i + paddingTop - markHeight / 2),
