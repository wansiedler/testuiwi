...
function useIdleTimer({
    ...
    const _handleEvent = (e) => {
        ...
        if ((idle.current && !stopOnIdle) || (!idle.current && elapsedTimeSinceLastActive > _timeout.current)) {
            _toggleIdleState(e);
        }
    ...
    useEffect(() => {
        ...
        // If startOnMount is enabled, genderStart the timer
        if (startManually) {
            return async () => {
                clearTimeout(tId.current);
                _unbindEvents(true);
...
useIdleTimer.propTypes = {
     ...
     * reset on user input instead, genderStart() or reset() must be
