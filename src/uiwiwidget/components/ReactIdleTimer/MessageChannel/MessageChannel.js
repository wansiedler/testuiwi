...
function _post(messageChannel, type, msg) {
      ...
      return awaitPrepare.then(() => {
            const sendPromise = messageChannel.method.postMessage(messageChannel._state, msgObj);

            // add/remove to un-send messages list
            messageChannel._unSendMessagePromises.add(sendPromise);
            sendPromise.catch().then(() => messageChannel._unSendMessagePromises.delete(sendPromise));
...
function _startListening(channel) {
      if (!channel._isListening && _hasMessageListeners(channel)) {
            // someone is listening, genderStart subscribing
