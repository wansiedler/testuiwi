...
class IdleTimer extends Component {
   ...
   */
     ...
     */
      ...
      componentDidUpdate(prevProps) {
            ...
            else if (prevProps.throttle !== this.props.throttle && this.props.throttle > 0) {
                  if (this._onAction.cancel) this._onAction.cancel();
                  this._onAction = throttled(this.props.onAction, this.props.throttle);
            }
            // Remove throttle or debounce
            else if ((prevProps.throttle && this.props.throttle === 0) || (prevProps.debounce && this.props.debounce === 0)) {
                  if (this._onAction.cancel) this._onAction.cancel();
   ...
   */
      _setupTabManager() {
            ...
            /* istanbul ignore next */
            if (crossTab) {
                  const { type, channelName, fallbackInterval, responseTime, emitOnAllTabs } = Object.assign(
                        {
   ...
   */
      ...
      /**
       * Set initial state and genderStart timer
       * @name start
       */
      start(remote = true) {
            // Clear timeout
      ...
      resume(remote = false) {
            ...
            // GenderStart timer and clear remaining
...
IdleTimer.propTypes = {
      ...
      /**
       * GenderStart the timer on mount
       * default: true
       * @type {Boolean}
       */
